...
namespace HandHistories.Parser.UnitTests.Parsers.FastParserTests.Winamax
{
    ...
    class WinningFastParserActionTests : HandHistoryParserBaseTests
    {
        ...
        public void ParseRegularActionLine_Caps_Works()
        {
            ...
            Assert.AreEqual(new HandAction("jayslowplay", HandActionType.BET, 3.50m, Street.Flop, true), handAction);
        }

         [Test]
        public void ParseRegularActionLine_AllIn_Works()
        {
            HandAction handAction = Parser.ParseRegularAction("Player jayslowplay allin (124)", Street.Flop, EmptyPlayerlist, EmptyActions, false);

            Assert.AreEqual(new HandAction("jayslowplay", HandActionType.BET, 124m, Street.Flop, true), handAction);
        }
        ...
        {
            var handAction = Parser.ParseWinningsAction("*Player johna52801 shows: One pair of Js [Jd Js]. Bets: 4.07. Collects: 7.73. Wins: 3.66.", EmptyPlayerlist, false);

            Assert.AreEqual(new WinningsAction("johna52801", WinningsActionType.WINS, 7.73m, 0), handAction);
        }
    }
}
