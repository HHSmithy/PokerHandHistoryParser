...
namespace HandHistories.CommonObjects
{
    ...
    public class Card
    {
        ...
        public Card(string rank, string suit)
        {
            ...
            if (!IsValidSuit(suit))
                throw new CardException("Suit is not correctly formatted. Should be c, d, h or s.");

            if (!IsValidRank(rank))
                throw new CardException("Rank is not correctly formatted. Should be 2-9, T, J, Q, K or A.");
        ...
        {
            if (card.Length != 2)
                throw new CardException("Cards must be length 2. Format Rs where R is rank and s is suit.");
